  ...
  <div class="liquidacion-caja">
    ...
    <section class="card entrega-banco">
      ...
      <form class="entrega-form" @submit.prevent="registrarEntregaBanco">
        <label>Importe<input v-model.number="entrega.importe" type="number" min="0.01" step="0.01" :max="saldoCaja" required /></label>
        ...
        <button class="btn-bank" :disabled="registrandoEntrega || !fincaId || entrega.importe <= 0 || entrega.importe > saldoCaja || !entrega.referenciaBancaria">{{ registrandoEntrega ? 'Registrando...' : 'Entregar al banco' }}</button>
      </form>
    </section>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in pendientes" :key="item.itemSalidaId" :class="{ seleccionado: estaSeleccionado(item.itemSalidaId) }">
              ...
              <td><input v-if="estaSeleccionado(item.itemSalidaId)" v-model.number="seleccionados[item.itemSalidaId].efectivo" type="number" min="0" :max="item.saldoPendiente" step="0.01" @input="normalizar(item.itemSalidaId)" /></td>
...
const solicitudesLiquidacion = computed<LiquidarSalidaRequest[]>(() => {
  ...
  for (const linea of lineasSeleccionadas.value) {
    ...
    if (linea.efectivo > 0) solicitud.aplicaciones.push({ itemSalidaId: linea.itemSalidaId, importe: linea.efectivo, formaPago: 'EFECTIVO' })
...
async function registrarEntregaBanco() {
  ...
  try {
    ...
    entrega.importe = 0; entrega.referenciaBancaria = ''; entrega.observaciones = ''
    await cargarDatos()
  } catch (error) {
...
.entrega-banco { background: #fffbeb; border-color: #fcd34d; }.entrega-form { display: grid; grid-template-columns: 1fr 1fr 1.5fr 1.5fr auto; gap: .75rem; align-items: end; }.entrega-form label { display: grid; gap: .25rem; font-size: .8rem; font-weight: 600; }.entrega-form input, table input { border: 1px solid #cbd5e1; border-radius: 5px; padding: .5rem; min-width: 0; }.btn-primary, .btn-secondary, .btn-bank { border: 0; border-radius: 6px; color: #fff; cursor: pointer; font-weight: 600; padding: .6rem 1rem; }.btn-primary { background: #2563eb; }.btn-secondary { background: #475569; }.btn-bank { background: #b45309; white-space: nowrap; }.btn-primary:disabled, .btn-secondary:disabled, .btn-bank:disabled { cursor: not-allowed; opacity: .55; }
.pending-count { background: #e0e7ff; border-radius: 99px; color: #3730a3; font-size: .8rem; font-weight: 700; padding: .35rem .65rem; }.table-wrap { overflow-x: auto; }table { border-collapse: collapse; width: 100%; min-width: 1120px; font-size: .84rem; }th, td { border-bottom: 1px solid #e2e8f0; padding: .6rem; text-align: left; vertical-align: middle; }th { background: #f1f5f9; color: #334155; font-size: .72rem; text-transform: uppercase; }td small { display: block; font-size: .72rem; margin-top: .15rem; }.seleccionado { background: #eff6ff; }.tipo { background: #e0e7ff; border-radius: 3px; color: #3730a3; display: inline-block; font-size: .65rem; font-weight: 700; margin-right: .3rem; padding: .15rem .3rem; }.money { text-align: right; font-variant-numeric: tabular-nums; font-weight: 600; }.error { color: #b91c1c; }.acciones { align-items: center; display: flex; justify-content: flex-end; margin-top: 1rem; }.validation-error { color: #b91c1c; margin-right: auto; }.empty { color: #64748b; padding: 2rem; text-align: center; }.empty.success { color: #15803d; }@media (max-width: 950px) { .resumen-grid, .entrega-form { display: grid; grid-template-columns: 1fr; }.page-header, .section-title, .header-actions { flex-direction: column; }.btn-secondary { align-self: stretch; } }
</style>
