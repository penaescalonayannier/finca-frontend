<!-- src/components/EditarTipoReporte.vue -->
<template>
  <div class="editar-tipo-reporte">
    <div class="formulario-contenedor">
      ...
      <form v-else @submit.prevent="guardarTipo" class="formulario">
        ...
        <div v-if="formulario.tipoSubclasificacion === 'CULTIVO'" class="grupo-formulario">
          ...
          >
            <option :value="undefined">No auto-seleccionar</option>
            <option
...
import type { TipoReporteRequest } from '@/types/TipoReporte'
...
const cargarTipo = async () => {
  ...
  try {
    const tipo = await TipoReporteService.getById(id)
    formulario.value = {
      codigo: tipo.codigo,
      nombre: tipo.nombre,
      descripcion: tipo.descripcion || '',
      codigoCentroCosto: tipo.codigoCentroCosto,
      tipoSubclasificacion: tipo.tipoSubclasificacion,
      tipoCultivoCategoriaFiltro: tipo.tipoCultivoCategoriaFiltro || null,
      tipoCultivoAutoId: tipo.tipoCultivoAutoId,
      requiereCampo: tipo.requiereCampo,
      activo: tipo.activo,
      orden: tipo.orden || 0
    }
  } catch (error) {
    console.error('Error al cargar tipo de reporte:', error)
    mensaje.value = { texto: 'Error al cargar el tipo de reporte', tipo: 'error' }
  } finally {
...
const guardarTipo = async () => {
  ...
  try {
    await TipoReporteService.update(id, formulario.value)
    mensaje.value = { texto: 'Tipo de reporte actualizado exitosamente', tipo: 'exito' }
    setTimeout(() => {
      router.push('/lista-tipo-reportes')
    }, 1500)
  } catch (error) {
    console.error('Error al actualizar tipo de reporte:', error)
    mensaje.value = { texto: 'Error al actualizar el tipo de reporte', tipo: 'error' }
  } finally {
