<!-- src/components/CrearCentroCosto.vue -->
<template>
  <div class="crear-centro-costo">
    <div class="header">
...
const cargarCuentasPadre = async () => {
  try {
    ...
    // Try to pre-select account 700 if it exists
    const cuenta700 = cuentas.find(c => c.codigo === '700')
    if (cuenta700) {
      form.value.cuentaPadreId = cuenta700.id
    }
  } catch (error) {
...
const guardar = async () => {
  ...
  try {
    // Always mark as cost center
    form.value.esCentroCosto = true

    await CuentaContableService.create(form.value)
    mensaje.value = { texto: 'Centro de costo creado exitosamente', tipo: 'exito' }
    ...
    const errorMsg = err.response?.data?.message || 'Error al crear el centro de costo'
